...
// ---------- Prefix Mod v5 (randomised startup assignments + stat injection) ----------
  ...
  const SYNERGY_CONFIG = {
    bonusByLine: { off: 5.5, mid: 5.2, def: 4.8 },
    targetByLine: { off: 6, mid: 6, def: 6 },
    minGroupSize: { off: 6, mid: 6, def: 6 }
  };
  ...
  function computeLineupSynergy() {
    ...
    const totals = { off: 0, mid: 0, def: 0 };
    const globalPrefixTotals = new Map();
    const prefixFirstIndex = new Map();
    let encounterIndex = 0;
    let placementsUsed = false;

    const registerPrefix = (line, prefix) => {
      if (!line || !prefix || !prefixMaps[line]) return;
      const map = prefixMaps[line];
      map.set(prefix, (map.get(prefix) || 0) + 1);
      globalPrefixTotals.set(prefix, (globalPrefixTotals.get(prefix) || 0) + 1);
      if (!prefixFirstIndex.has(prefix)) {
        prefixFirstIndex.set(prefix, encounterIndex++);
      }
    };
    ...
    if (placements && placements.length) {
      placements.forEach(({ prefix, line, bench }) => {
        if (!line || bench || !prefixMaps[line]) return;
        placementsUsed = true;
        totals[line] += 1;
        registerPrefix(line, prefix);
      });
    }
    ...
    if (!placementsUsed) {
      ...
      cards.forEach(card => {
        applyCardMetadata(card);
        const line = card.dataset.modPrefixLine;
        if (!line || !prefixMaps[line]) return;
        const context = card.dataset.modPrefixContext;
        if (context && context !== 'lineup') return;
        totals[line] += 1;
        const prefixName = (card.dataset.modPrefixName || card.dataset.modPrefix || '').trim();
        registerPrefix(line, prefixName);
      });
    }

    const summary = { off: null, mid: null, def: null };
    Object.keys(prefixMaps).forEach(line => {
      const map = prefixMaps[line];
      const counts = Array.from(map.entries());
      const totalCards = totals[line] || counts.reduce((acc, [, count]) => acc + count, 0);
      if (!totalCards) {
        summary[line] = null;
        return;
      }
      ...
      let leadingPrefix = null;
      const candidates = counts.filter(([, count]) => count === leadingCount);
      if (candidates.length === 1) {
        leadingPrefix = candidates[0][0];
      } else {
        const ranked = candidates
          .map(([prefix, count]) => {
            const globalTotal = globalPrefixTotals.get(prefix) || count;
            const ratio = globalTotal > 0 ? count / globalTotal : 0;
            const firstIndex = prefixFirstIndex.has(prefix)
              ? prefixFirstIndex.get(prefix)
              : Number.MAX_SAFE_INTEGER;
            const linePresence = Object.keys(prefixMaps).reduce(
              (acc, key) => acc + (prefixMaps[key].has(prefix) ? 1 : 0),
              0
            );
            return { prefix, count, ratio, globalTotal, firstIndex, linePresence };
          })
          .sort((a, b) => {
            if (b.ratio !== a.ratio) return b.ratio - a.ratio;
            if (a.linePresence !== b.linePresence) return a.linePresence - b.linePresence;
            if (b.count !== a.count) return b.count - a.count;
            if (a.globalTotal !== b.globalTotal) return a.globalTotal - b.globalTotal;
            return a.firstIndex - b.firstIndex;
          });
        leadingPrefix = ranked.length ? ranked[0].prefix : candidates[0][0];
      }
      ...
      const target = SYNERGY_CONFIG.targetByLine[line] || 0;
      const minSize = SYNERGY_CONFIG.minGroupSize[line] || target || 0;
      const requiredCount = Math.max(minSize, target || 0);
      const leadingTotal = map.get(leadingPrefix) || 0;
      const allSame = leadingTotal === totalCards && totalCards > 0;
      const active = allSame && leadingTotal >= requiredCount;
      summary[line] = {
        ...
        bonus: active ? (SYNERGY_CONFIG.bonusByLine[line] || 0) : 0,
        active
      };
    });
    if (typeof window !== 'undefined') {
  ...
  function renderFieldSynergyPanel(summary) {
    ...
    order.forEach(({ key, label }) => {
      ...
      if (data && data.prefix) {
        hasAny = true;
        chip.textContent = data.prefix;
        const prefixMeta = availablePrefixes.find(p => p && p.name === data.prefix);
        accentColor = prefixMeta ? getAccentColor(prefixMeta) : null;
        if (prefixMeta && prefixMeta.labelBackground) {
          chip.style.setProperty('--mod-prefix-field-chip-bg', prefixMeta.labelBackground);
        } else {
          chip.style.removeProperty('--mod-prefix-field-chip-bg');
        }
        if (prefixMeta && prefixMeta.labelColor) {
          chip.style.setProperty('--mod-prefix-field-chip-color', prefixMeta.labelColor);
        } else {
        ...
        const totalCards = Number.isFinite(totalValue) ? totalValue : count;
        const cappedCount = target ? Math.min(count, target) : count;
        progress.textContent = target ? `${cappedCount}/${target}` : `${count}`;
        progress.dataset.progress = progress.textContent;
        const neededForTarget = target ? Math.max(0, target - count) : 0;
        const neededForUniform = Math.max(0, totalCards - count);
        const needed = Math.max(neededForTarget, neededForUniform);
        ...
        if (data.active && Number.isFinite(bonusValue) && bonusValue !== 0) {
          ...
          status.textContent = `need ${needed}`;
        ...
        chip.textContent = '—';
        const target = SYNERGY_CONFIG && SYNERGY_CONFIG.targetByLine
          ? SYNERGY_CONFIG.targetByLine[key] || 0
          : 0;
        progress.textContent = target ? `0/${target}` : '0';
        progress.dataset.progress = progress.textContent;
        status.textContent = '';
  ...
  function applySynergyToAnalysis(element, synergyData) {
    ...
    if (!synergy || !synergy.prefix || !Number.isFinite(Number(synergy.count))) {
      return;
    }
    const prefixName = synergy.prefix;
    const rawCount = Number(synergy.count) || 0;
    if (rawCount <= 0) {
      return;
    }
    const totalValue = Number(synergy.total);
    const totalCards = Number.isFinite(totalValue) ? totalValue : rawCount;
    const target = Number(synergy.target) || 0;
    const cappedCount = target ? Math.min(rawCount, target) : rawCount;
    const neededForTarget = target ? Math.max(0, target - rawCount) : 0;
    const neededForMismatch = Math.max(0, totalCards - rawCount);
    const needed = Math.max(neededForTarget, neededForMismatch);
    ...
    element.dataset.modPrefixSynergyPrefix = prefixName;
    ...
    element.dataset.modPrefixSynergyNeeded = String(needed);
